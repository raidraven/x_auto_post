...
def page_search():
    ...
    with st.expander("絞り込み設定"):
        col1, col2 = st.columns(2)
        with col1:
            min_followers = st.number_input("フォロワー最小", value=100, step=100, min_value=0)
            hours_within = st.selectbox("投稿時間", [6, 12, 24, 48, 72], index=2, format_func=lambda x: f"{x}時間以内")
        with col2:
            max_followers = st.number_input("フォロワー最大", value=50000, step=1000, min_value=1)
    ...
    if st.session_state.tweets:
        st.markdown(f"**{len(st.session_state.tweets)}件見つかりました（エンゲージメント率順）**")
        st.divider()

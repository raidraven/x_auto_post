...
def save_markdown(tweets: list[dict], output_dir: str = "X投稿データ") -> str:
    ...
    lines = [
        ...
        f"| いいね | {total_likes} |",
        f"| リポスト | {total_reposts} |",
        f"| 引用 | {total_quotes} |",
        f"| リプライ | {total_replies} |",
        f"| ブックマーク | {total_bookmarks} |",
        "",
        "## エンゲージメント上位3件",
        "",
    ]
    ...
    for i, t in enumerate(ranked[:3], 1):
        lines += [
            ...
            f"> {t['text']}",
            "",
            f"| いいね | RT | 引用 | リプライ | ブックマーク | インプレッション |",
            f"|--------|-----|------|----------|--------------|------------------|",
            f"| {t['likes']} | {t['reposts']} | {t['quotes']} | {t['replies']} | {t['bookmarks']} | {t['impressions']} |",
            "",
        ]
    ...
    for t in tweets:
        lines.append(f"| {t['created_at']} | {t['text']} | {t['impressions']} | {t['likes']} | {t['reposts']} | {t['replies']} |")
...
if __name__ == "__main__":
    ...
    if tweets:
        csv_path = save_csv(tweets)
        print(f"CSVを保存しました: {csv_path}")
        md_path = save_markdown(tweets)
        print(f"レポートを保存しました: {md_path}")
